...
var _DateUtil = class _DateUtil {
  ...
  now() {
    ...
    return (0, import_date_fns_tz.toZonedTime)(date, _DateUtil.timezone);
  }
  /**
  ...
  format(date, pattern = "yyyy-MM-dd HH:mm:ss") {
    return (0, import_date_fns_tz.formatInTimeZone)(date, _DateUtil.timezone, pattern);
  }
  /**
   ...
   */
  addDays(date, days) {
    return (0, import_date_fns.addDays)(date, days);
  }
  /**
   ...
   */
  subDays(date, days) {
    return (0, import_date_fns.subDays)(date, days);
  }
  /**
   ...
   */
  addHours(date, hours) {
    return (0, import_date_fns.addHours)(date, hours);
  }
  /**
   ...
   */
  subHours(date, hours) {
    return (0, import_date_fns.subHours)(date, hours);
  }
  /**
  ...
  addMinutes(date, minutes) {
    return (0, import_date_fns.addMinutes)(date, minutes);
  }
  /**
  ...
  subMinutes(date, minutes) {
    return (0, import_date_fns.subMinutes)(date, minutes);
  }
  /**
   ...
   */
  isFuture(date) {
    const zonedDate = (0, import_date_fns_tz.toZonedTime)(date, _DateUtil.timezone);
    return (0, import_date_fns.isFuture)(zonedDate);
  }
  /**
  ...
  isPast(date) {
    const zonedDate = (0, import_date_fns_tz.toZonedTime)(date, _DateUtil.timezone);
    return (0, import_date_fns.isPast)(zonedDate);
  }
  /**
  ...
  isToday(date) {
    const zonedDate = (0, import_date_fns_tz.toZonedTime)(date, _DateUtil.timezone);
    const todayZoned = (0, import_date_fns_tz.toZonedTime)(/* @__PURE__ */ new Date(), _DateUtil.timezone);
    return (0, import_date_fns.isSameDay)(zonedDate, todayZoned);
  }
  /**
  ...
  isSameDay(a, b) {
    ...
    return (0, import_date_fns.isSameDay)(zonedA, zonedB);
  }
  /**
  ...
  startOfDay(date) {
    const zonedDate = (0, import_date_fns_tz.toZonedTime)(date, _DateUtil.timezone);
    const start = (0, import_date_fns.startOfDay)(zonedDate);
    return (0, import_date_fns_tz.fromZonedTime)(start, _DateUtil.timezone);
  }
  /**
  ...
  endOfDay(date) {
    const zonedDate = (0, import_date_fns_tz.toZonedTime)(date, _DateUtil.timezone);
    const end = (0, import_date_fns.endOfDay)(zonedDate);
    return (0, import_date_fns_tz.fromZonedTime)(end, _DateUtil.timezone);
  }
  /**
   * Compares two dates.
   * @param a The first date.
   ...
   */
  compareDates(a, b) {
    return (0, import_date_fns.compareAsc)(a, b);
  }
  /**
   ...
   */
  daysBetween(a, b) {
    const diff = Math.abs((0, import_date_fns.startOfDay)(a).getTime() - (0, import_date_fns.startOfDay)(b).getTime());
   ...
   */
  toUnix(date) {
    return (0, import_date_fns.getUnixTime)(date);
  }
  /**
   ...
   */
  fromUnix(unix) {
    return (0, import_date_fns.fromUnixTime)(unix);
  }
  /**
...
Object.assign(import_lodash.default, {
...
var AwsAdapter = class {
  ...
  async uploadFile(bufferOrStream, options) {
    ...
    await client.send(new import_client_s3.PutObjectCommand(uploadParams));
...
var GcpAdapter = class {
  getStorageClient(credentials) {
    ...
    if (gcpKeyFilePath) {
      return new import_storage.Storage({ keyFilename: gcpKeyFilePath, projectId });
    }
    return new import_storage.Storage({ projectId });
  }
  async uploadFile(bufferOrStream, options) {
    const { key, mimeType, bucket, credentials } = options;
    ...
    await new Promise((resolve, reject) => {
      ...
      if (bufferOrStream instanceof Buffer) {
        const bufferStream = import_stream.Readable.from(bufferOrStream);
...
var AzureAdapter = class {
  getBlobServiceClient(credentials) {
    ...
    return import_storage_blob.BlobServiceClient.fromConnectionString(azureConnectionString);
  }
  async uploadFile(bufferOrStream, options) {
    const { key, mimeType, bucket, credentials } = options;
  ...
  async generatePresignedUrl(options) {
    ...
    const sasOptions = {
      ...
      permissions: import_storage_blob.BlobSASPermissions.from({ write: true }),
      // Write permission
      contentType: mimeType
    };
